...
func (s *Server) handleProjects(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.renderer.Render(w, "projects", map[string]any{"Title": "Projects", "Projects": projects}); err != nil {
		http.Error(w, "template render failed", http.StatusInternalServerError)
	}
}

func (s *Server) handleProject(w http.ResponseWriter, r *http.Request) {
	if err := s.renderer.Render(w, "project", map[string]any{"Title": "Project"}); err != nil {
		http.Error(w, "template render failed", http.StatusInternalServerError)
	}
}

func (s *Server) handleAssessment(w http.ResponseWriter, r *http.Request) {
	if err := s.renderer.Render(w, "assessment", map[string]any{"Title": "Assessment"}); err != nil {
		http.Error(w, "template render failed", http.StatusInternalServerError)
	}
}

func (s *Server) handleResults(w http.ResponseWriter, r *http.Request) {
	if err := s.renderer.Render(w, "results", map[string]any{"Title": "Results"}); err != nil {
		http.Error(w, "template render failed", http.StatusInternalServerError)
	}
}

func (s *Server) handleReport(w http.ResponseWriter, r *http.Request) {
	if err := s.renderer.Render(w, "report", map[string]any{"Title": "Report"}); err != nil {
		http.Error(w, "template render failed", http.StatusInternalServerError)
	}
}
